...
ELClientPacket* ELClient::protoCompletedCb(void) {
  ...
  if (crc != resp_crc) {
    if (_debugEn) _debug->println("ELC: Invalid CRC");
    return NULL;
  }
  ...
  if (packet->cmd == CMD_RESP_V) {
    ...
    if (fp->attached()) {
      ...
      _proto.dataLen = 0;    // <<<---- added
      _proto.isEsc = 0;      // <<<---- added
      (*fp)(&resp);
    }
    return NULL;
  } else {
...
bool ELClient::Sync(uint32_t timeout) {
  ...
  while ((packet = WaitReturn(timeout)) != NULL) {
    if (packet->value == (uint32_t)&wifiCb) { if (_debugEn) _debug->println("SYNC!"); return true; }
    if (_debugEn) {
      _debug->print("BAD: ");
      _debug->println(packet->value);
    }
